...
module.exports = function (context) {
  ...
  function checkKeysTree(_keysTree) {
    var combos = getCombinations(Object.keys(_keysTree).filter(function (item) {return item !== nodesField;}), 2);
    combos.forEach(function (combo) {
      if (combo[0].length <= 5 || combo[1].length <= 5 || combo[0] === combo[1]) {
        return;
      }
      var distance = s.getLevenshteinDistance.apply(null, combo);
      if (distance <= 2) {
