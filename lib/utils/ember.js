 ...
 * @private
 */
function _isEmber(str) {
  return ["Em", "Ember"].indexOf(str) !== -1;
}
...
function isComputedMacro(node) {
  ...
  if (calleeType === "MemberExpression") {
    ...
    if (o.get(node, "callee.object.type") === "MemberExpression") {
      ...
      if (_isEmber(coon) && copn === "computed") {
        return true;
      }
    }
  }
...
function _isEmberFieldByType(node, defaultName, extendedName) {
  ...
  var calleeType = o.get(node, "callee.type");
  if (calleeType === "MemberExpression") {
    if (_isEmber(o.get(node, "callee.object.name")) && (o.get(node, "callee.property.name") === defaultName || o.get(node, "callee.property.value") === defaultName)) {
      return true;
    }
